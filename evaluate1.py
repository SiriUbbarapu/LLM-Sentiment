...
def plot_confusion_matrix(true_labels, pred_labels, label_space, dataset_name):
    cm = confusion_matrix(true_labels, pred_labels, labels=label_space)
    plt.figure(figsize=(10, 7))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=label_space, yticklabels=label_space)
...
def calculate_metric_and_errors(task, dataset, df):
    ...
    plot_confusion_matrix(true_labels, pred_labels, label_space, dataset)
        ...
        for dataset in sorted(os.scandir(task_output_folder), key=lambda e: e.name):
            if dataset.is_dir():
                if selected_datasets is None or dataset.name in selected_datasets:
                    dataset_output_folder = os.path.join(task_output_folder, dataset.name)
                    print(f"\nProcessing: {dataset.name}")
                    metric_dict[dataset.name] = process_file(task, dataset.name, dataset_output_folder)
