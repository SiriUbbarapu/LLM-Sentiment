...
def calculate_metric_and_errors(task, dataset, df):
    ...
    # Save the figure
    output_dir = os.path.dirname(os.path.abspath(df.name))  # Get the directory of the input file
    plt.savefig(os.path.join(output_dir, f'{dataset}_confusion_matrix.png'))
    
    # Close the figure to free up memory
    plt.close(fig)

    print(f"Confusion matrix for {dataset} saved as '{dataset}_confusion_matrix.png'")
