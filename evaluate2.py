...
from sklearn.metrics import (
    ...
    ConfusionMatrixDisplay  # Import ConfusionMatrixDisplay for visualization
)
from predict import get_label_space
import argparse
import matplotlib.pyplot as plt  # Import matplotlib for plotting the confusion matrix
...
def calculate_metric_and_errors(task, dataset, df):
    ...
    metric_name = ""  # Initialize metric_name
    metric = 0.0  # Initialize metric
    
    # Perform metrics calculation as before
    if task == "sc":
    ...
    plt.savefig(f'confusion_matrix_{dataset}.png')  # Save as a PNG file
